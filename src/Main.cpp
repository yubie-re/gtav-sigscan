...
std::string DownloadTunables(const std::string& platform = "pcrosalt")
{
    ...
    const size_t blockSize = cipher->block_size(); // should be 16
    const size_t blockCount = r.text.size() / blockSize;
    cipher->set_key(g_TunableKey);
    cipher->decrypt_n(reinterpret_cast<uint8_t*>(r.text.data()), reinterpret_cast<uint8_t*>(r.text.data()), blockCount);   
    return r.text;
}
...
Botan::secure_vector<uint8_t> GetAnticheatData(rapidjson::Document& d)
{
    if (!IsNewFormat(d)) {
        return {};
    }
    if (d["tunables"]["8B7D3320"].IsString())
        return Botan::base64_decode(d["tunables"]["8B7D3320"].GetString());
    return Botan::base64_decode(d["tunables"]["8B7D3320"][0]["value"].GetString());
}
...
uint32_t FNV1a(std::span<const uint8_t> data)
{
    ...
    for (uint8_t byte : data) {
        hash = 0x1000193 * (byte ^ hash);
    }
    return hash;
}

uint32_t JoaatData(std::span<const uint8_t> data)
{
...
size_t ScanBuffer(std::span<const uint8_t> data, const ScanJob& sig)
{
    ...
    auto end = data.end() - sigLen + 1;

    while (begin != data.end() && begin != end) {
        begin = std::find_if(begin, end, [&](uint8_t b) {
            return b == sig.m_FirstByte;
        });

        if (begin == end)
            break;

        size_t offset = std::distance(data.begin(), begin);
        std::span<const uint8_t> slice = data.subspan(offset, sigLen);

        if ((!g_Old && FNV1a(slice) == sig.m_Hash) ||
            (g_Old && JoaatData(slice) == sig.m_Hash))
            return offset;

        ++begin; // Continue searching from the next byte
    }
    ...
    SignatureType& signature)
{
    ...
    if (size_t location = ScanBuffer(data, ScanJob({ signature.m_FirstByte, signature.m_Len, signature.m_Hash }))) {
        ...
        auto signatureSpan = std::span(data.begin() + location, signature.m_Len);
        ...
        if (IsAscii(signatureSpan)) {
            std::string asciiResult(reinterpret_cast<const char*>(data.data()) + location,
                signature.m_Len);
...
void ProcessSigs(Botan::secure_vector<uint8_t>& acData)
{
    for (int i = 8; i + 0x21 < acData.size(); i += 0x20) {
        switch (acData[i++]) {
        case ANTICHEAT_RTMA: {
            RTMASig rtma;
            if(acData.size() < i + 0x20) {
                fmt::print("RTMA signature data is too short!\n");
                return;
            }
            std::span<uint8_t> chunk(acData.begin() + i, acData.begin() + i + 0x20);
            NG::DecryptNG(chunk, g_RtmaKey);
            rtma = *reinterpret_cast<RTMASig*>(chunk.data());
        ...
        case ANTICHEAT_INTEG: {
            IntegSig integ;
                        std::span<uint8_t> chunk(acData.begin() + i, acData.begin() + i + 0x20);
            NG::DecryptNG(chunk, g_IntgKey);
            integ = *reinterpret_cast<IntegSig*>(chunk.data());
...
int main(int argc, const char* argv[])
{
    ...
    options.add_options()
      ...
      ("time", "Sets the time exported to the json",cxxopts::value<time_t>(), "<time>");
        ...
        if(!std::filesystem::exists(result["directory"].as<std::string>())) {
            fmt::print("Created dir {}.\n", result["directory"].as<std::string>());
            std::filesystem::create_directory(result["directory"].as<std::string>());
        }
        ...
        if (!g_Old && IsNewFormat(d)) {
            Botan::secure_vector<uint8_t> data = GetAnticheatData(d);
            if (data.empty() || data.size() < 8) {
                fmt::print("Download failed\n");
                return 0;
            }
            NG::DecryptNG(data, g_DecKey);
