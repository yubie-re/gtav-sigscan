...
namespace NG
{
    ...
    template<typename Iter>
    void DecryptNGRoundA(ArxanBlock<Iter>& data, const ArxanRoundKey& key, const std::array<std::array<uint32_t, 256>, 16>& table);
    template<typename Iter>
    void DecryptNGRoundB(ArxanBlock<Iter>& data, const ArxanRoundKey& key, const std::array<std::array<uint32_t, 256>, 16>& table);

    void DecryptNG(std::vector<uint8_t>& data, const ArxanKey& key)
    {
        for(size_t blockIndex = 0; blockIndex < data.size() / 16; blockIndex++)
        {
            ArxanBlock block = std::ranges::subrange(data.begin() + 16 * blockIndex, data.begin() + 16 * (blockIndex + 1));
    ...
    void DecryptNGRaw(uint8_t* data, size_t size, const ArxanKey& key)
    {
        ...
        {
            ArxanBlock block = std::ranges::subrange(data + 16 * blockIndex, data + 16 * (blockIndex + 1));
            DecryptNGBlock(block, key);
        }
    }
